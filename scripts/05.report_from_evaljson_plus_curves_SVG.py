...
def fig_save(path: Path, also_svg: bool = True):
    """Save figure as PNG (and optionally SVG) with tight layout."""
    path.parent.mkdir(parents=True, exist_ok=True)
    plt.tight_layout()

    # Always save PNG (raster)
    plt.savefig(path, dpi=300, bbox_inches="tight")

    # Save SVG (vector) alongside, if requested
    if also_svg:
